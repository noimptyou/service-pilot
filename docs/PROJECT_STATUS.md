...
> 最后更新：2026-09-01
> 当前分支：`master`  
> 当前阶段：SSE 消息实时推送已完成自动化测试和浏览器真实联调，下一步开发人工客服工作台前端
...
- 使用 SSE 订阅会话消息与状态变化，管理员发送 `AGENT` 消息后触发页面静默同步。
- 已移除转人工期间每 4 秒轮询，浏览器断线时由 `EventSource` 自动重连。
...
- 人工客服短消息气泡随内容收缩，长消息受最大宽度限制并自动换行；AI 与知识引用区域继续保持稳定宽度。
...
- 客户页面为当前会话建立一个 SSE 长连接。
- 保存 `CUSTOMER`、`AI`、`AGENT` 或 `SYSTEM` 消息后发布 `message-created` 事件。
- 转人工申请、人工接单、申请完成和会话关闭后发布 `conversation-state-changed` 事件。
- 事务事件监听器仅在数据库事务提交成功后推送，避免页面收到尚未落库的数据。
- SSE 事件只作为“数据已变化”的通知，前端收到后重新读取聊天记录和最新转人工状态，数据库仍是最终数据源。
- 前端对并发到达的事件进行刷新排队，避免刷新过程中丢失后续变化。
- 订阅不存在的会话返回 HTTP 404，会话退出或结束时主动关闭连接。
...
- SSE 会话订阅成功进入异步响应，且返回 `text/event-stream`。
- 不存在的会话无法建立 SSE 订阅并返回 HTTP 404。
...
2026-09-01 完成 SSE 消息实时推送验证：Java 编译成功，42 个后端测试全部通过，0 失败、0 错误；`vue-tsc -b && vite build` 成功；浏览器真实联调确认人工接单状态和人工客服消息可自动显示，无需点击刷新或等待轮询。

2026-09-01 调整人工客服消息气泡宽度：短消息不再横向占满内容区，长消息保持最大宽度并自动换行；前端 `vue-tsc -b && vite build` 及浏览器页面验证均通过。
...
当前已实现：**SSE 消息实时推送**，已通过后端自动化测试、前端生产构建和浏览器真实联调。
...
下一项独立功能：开发**人工客服工作台前端**，用于查看待接单会话、接单、查看聊天上下文和发送人工消息。
...
```text
81d10f5 实现客户聊天前端完整交互
```

当前工作区包含待提交的 SSE 后端、前端实时订阅、人工消息气泡样式、自动化测试和进度文档修改；自动化验证与浏览器真实联调均已通过，可以提交。
